...
void Character::getItemFromDatabase(int itemID)
{
	...
	void Character::fillAbilitiesList()
	{
		//this int is used just to see if the vector 
		
		int levelObtained, damage, MPcost, dtype, AOE, GodAbility, active;
		std::string name;
		...
		while (!infile.eof)
		{
			...
			abilities.push_back(new Abilities(levelObtained, damage, MPcost, static_cast<Abilities::DamageType>(dtype), static_cast<bool>(AOE), static_cast<bool>(GodAbility), 
				static_cast<bool>(active), name));
